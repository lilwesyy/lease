<template>
    ...
    <Dialog v-model:visible="showDialog" header="Booking Details" :modal="true" class="p-fluid">
...
const statusTemplate = (slotProps) => {
    const severity = {
        'Confirmed': 'success',
        'Pending': 'warning',
        'Cancelled': 'danger',
...
const paymentStatusTemplate = (slotProps) => {
    const severity = {
        'Paid': 'success',
        'Unpaid': 'danger',
        'Partial': 'warning',
...
const editBooking = () => {
    if (selectedRow.value) {
        router.push(`/dashboard/edit-booking/${selectedRow.value.id}`);
    }
};
