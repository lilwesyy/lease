<template>
    ...
    <template #content>
      <TabView v-model:activeIndex="activeTabIndex">
        <TabPanel>
          ...
          <DatePicker v-model="dates" showIcon fluid iconDisplay="input" selectionMode="range"
            placeholder="Select Dates" :manualInput="false" @date-select="onDateSelect" class="w-full" />
          <div class="input-group-row grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            <InputGroup v-tooltip="'Select the vehicle from the table to set delivery location'">
              ...
              <Select v-model="deliveryLocation" :options="locations" optionLabel="label"
                placeholder="Delivery Location" class="w-full" />
            </InputGroup>
            <InputGroup>
              <InputGroupAddon>
                <i class="pi pi-map-marker"></i>
          ...
          <a href="#" class="add-location-link mt-2 block text-center" @click.prevent="openLocationModal">Missing
            location? Add
            New</a>

          <IconField class="p-d-flex p-ai-center mt-5 w-full input-container">
            <InputIcon class="pi pi-search" />
            <InputText v-model="vehicleSearch" placeholder="Search Vehicles" class="vehicle-input" />
            <Button label="Add Vehicle" class="ml-3 button-mobile-mode" icon="pi pi-plus"
              @click="$router.push('/dashboard/add-vehicles')" />
          </IconField>

          <div v-if="vehicles.length > 0">
            <DataTable :value="filteredVehicles" selectionMode="single" class="p-datatable-no-gridlines w-full"
              @row-click="onRowClick" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]"
              paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown">
              <Column header="Select" style="width: 50px">
                <template #body="slotProps">
                  <RadioButton v-model="selectedVehicleId" :inputId="`vehicle-${slotProps.data.id}`"
                    :name="`selectedVehicle`" :value="slotProps.data.id"
              ...
              <Column header="Brand and Model">
                <template #body="slotProps">
                  <div style="display: flex; align-items: center">
                    <img :src="slotProps.data.make?.icon" :alt="`${slotProps.data.make?.name} logo`" width="90"
                      height="90" v-if="slotProps.data.make?.icon" style="margin-right: 8px;" />
                    <div>
          ...
          <div class="flex justify-content-end mt-4">
            <Button label="Next" icon="pi pi-arrow-right" iconPos="right" :disabled="!isDateVehicleTabComplete"
              @click="goToCustomerTab" class="p-button-primary" />
          </div>
        </TabPanel>
        ...
        <TabPanel :disabled="!isDateVehicleTabComplete">
          ...
          <IconField class="p-d-flex p-ai-center mb-3 w-full">
            <InputIcon class="pi pi-search" />
            <InputText v-model="clientSearch" placeholder="Search Customers" @keyup.enter="searchClients"
              class="w-full" />
            <Button label="Add New Customer" icon="pi pi-plus" @click="$router.push('/dashboard/add-customer')"
              class="ml-3" />
          </IconField>

          <div v-if="clients.length > 0">
            <DataTable :value="filteredClients" selectionMode="single" dataKey="id" @row-click="onClientRowClick"
              class="p-datatable-no-gridlines w-full" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 20, 50]"
              paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown">
              <Column header="Select" style="width: 50px">
                <template #body="slotProps">
                  <RadioButton v-model="selectedClientId" :inputId="`client-${slotProps.data.id}`"
                    :name="`selectedClient`" :value="slotProps.data.id" />
                </template>
              </Column>
              <Column header="Name">
                <template #body="slotProps">
          ...
          <div class="flex justify-content-between mt-4">
            <Button label="Previous" icon="pi pi-arrow-left" class="p-button-secondary" @click="goToDateVehicleTab" />
            <Button label="Next" icon="pi pi-arrow-right" iconPos="right" :disabled="!isCustomerTabComplete"
              @click="goToSummaryTab" class="p-button-primary" />
          </div>
        </TabPanel>
        ...
        <TabPanel :disabled="!isCustomerTabComplete">
          ...
          <div v-if="selectedVehicleId && selectedClient && dates">
            ...
            <div class="summary-row grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="summary-column">
                ...
                <div v-if="selectedVehicle">
                  <div><strong>Make & Model:</strong> {{ selectedVehicle.make?.name }} {{ selectedVehicle.model?.name }}
                  </div>
            ...
            <div class="flex justify-content-between mt-4">
              <Button label="Previous" icon="pi pi-arrow-left" class="p-button-secondary" @click="goToCustomerTab" />
              <Button label="Confirm Booking" icon="pi pi-check" @click="confirmBooking" class="p-button-success" />
            </div>
          </div>
          <div v-else>
  ...
  <!-- Add Location Dialog -->
  <Dialog v-model:visible="showLocationDialog" header="Add New Location" :style="{ width: '450px' }" :modal="true"
    :closable="true">
...
/* Mantieni gli elementi sulla stessa riga (su schermi grandi) */
.input-container {
  display: flex;
  align-items: center;
  justify-content: space-between;
...
/* Permetti all'input di occupare la maggior parte della larghezza disponibile */
.vehicle-input {
  flex-grow: 1;
}
